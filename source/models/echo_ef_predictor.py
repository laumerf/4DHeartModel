...
# ------------------------------------------------------- Generators --------------------------------------------------------
class Echo_EF_FFNN(Layer):

    def __init__(self, dense_layers, name='Echo_EF_FFNN', **kwargs):
        super(Echo_EF_FFNN, self).__init__(name=name, **kwargs)
...
class Echo_EF_Predictor(Model):

    def __init__(self, log_dir, training_params, model_params, save_metrics=True, name='Echo_EF_Predictor', **kwargs):

        super(Echo_EF_Predictor, self).__init__(name=name, **kwargs)
        ...
        # build the Echo EF Predictor model
    ...
    def build_model(self, model_params):
        self.ef_ffnn = Echo_EF_FFNN(dense_layers=model_params['dense_layers'])
        ...
        for latent_params, true_efs in train_dataset:
            ...
            if global_step % num_steps == 0:
                ...
                if val_loss >= opt_early_stopping_metric:
                    ...
                    if count > 50:
                        logger.info("Model didn't improve during the last 50 steps. Stop model training")
                        break
    ...
    def plot_efs(self, dataset, type_folder, global_step):
        ...
        # ef correlation plot 
        plots_dir = self.log_dir / 'plots' / type_folder
